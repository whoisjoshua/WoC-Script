...
#############      SELECT CHAR       ##############
###################################################
#def select_char(draft_chars, chars):
...
if __name__ == '__main__':
    ...
    while (True):
        ...
        if (draft_stage == 1) and (draft_chars <= 2):
            if (draft_chars == 0):
                coords_char, found_char = getCoordinates(img_bgr, chars[0][1], 0.99)
                if (found_char == 1):
                    draft_chars += 1
                    detected_obj = chars[0][0]
                    coords = coords_char
                    subprocess.call('adb shell input tap %d %d' % (coords[0], coords[1]))
                    confirmation()
            elif (draft_chars == 1):
        ...
        # 3 ==> b2
        elif (draft_stage == 3) and (draft_chars <= 3):
            pass

        # 4 ==> r2
        elif (draft_stage == 4) and (draft_chars <= 2):
            pass
        ...
        # Second Drafts
        '''
        coords, found_b2 = getCoordinates(img_bgr, blue_2nd_draft, 0.99)
        coords, found_r2 = getCoordinates(img_bgr, red_2nd_draft, 0.99)
        if found_b2 == 1:
            draft_stage = 3
