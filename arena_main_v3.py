...
if __name__ == '__main__':
    ...
    while (True):
        ...
        elif (not draft_process):
            auto = 1
            coords_auto, found_auto = getCoordinates(img_bgr, auto_btn[1], 0.75)
            coords_replay, found_replay = getCoordinates(img_bgr, replay_btn[1], 0.99)
            if (found_auto) and (auto):
                detected_obj = auto_btn[0]
                coords = coords_auto
                subprocess.call('adb shell input tap %d %d' % (coords_auto[0], coords_auto[1]))
                auto = 0
                #OR tap can be at coords input tap 50 890
            if (found_replay):
                detected_obj = replay_btn[0]
                coords = coords_replay
                subprocess.call('adb shell input tap %d %d' % (coords_replay[0], coords_replay[1]))
                draft_process = 1
